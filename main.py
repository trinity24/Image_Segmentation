...
if __name__ == '__main__':
    ...
    superpixels = np.where(graph_cut[superpixels], 500, superpixels)
    superpixels = np.where((superpixels != 500), 0, superpixels)
    superpixels = np.where((superpixels != 0), 255, superpixels)

    # target = cv2.imread("target.png", 0)
    # print RMSD(np.array(target, dtype=np.uint8), np.array(superpixels, dtype=np.uint8))

    output_name = sys.argv[3] + "mask.png"
    cv2.imwrite(output_name, superpixels)
